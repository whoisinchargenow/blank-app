...
# --- MODIFICATION: New helper function for Google Search ---
def construct_google_search_url(image_url: str) -> str:
    """Constructs a URL for Google's reverse image search."""
    encoded_url = quote(image_url)
    return f"https://www.google.com/searchbyimage?image_url={encoded_url}"
...
if uploaded_file:
    ...
    with st.spinner("Searching..."):
        try:
            # --- MODIFICATION: Store the public URL for Google Search ---
            query_url_for_google = upload_query_image_to_r2(img_bytes, uploaded_file.name)
        except Exception as e:
            st.error(f"Failed to upload image: {e}"); st.stop()
        
        vis_res = marqo_search(query_url_for_google, attrs=[IMAGE_FIELD], filter_string=color_filter_string)
        sem_res = marqo_search(query_url_for_google, attrs=[SEARCH_BLOB_FIELD], filter_string=color_filter_string)
...
# --- MODIFICATION: Create a two-column layout for results ---
marqo_col, google_col = st.columns([3, 1])
...
with marqo_col:
    if final_hits:
        ...
        st.subheader(f"Found {len(final_hits)} results from Marqo")
        page_size = 9
        ...
        st.warning("No results found in Marqo. Try adjusting your query or the filter settings.")

# --- MODIFICATION: New right-hand panel for Google Search results ---
with google_col:
    if query_url_for_google:
        st.subheader("Google Search")
        st.markdown("---")
        google_url = construct_google_search_url(query_url_for_google)
        st.markdown(f'''
        <a href="{google_url}" target="_blank" style="text-decoration: none;">
            <div style="padding: 1rem; border: 1px solid #444; border-radius: 0.5rem; text-align: center;">
                <img src="https://upload.wikimedia.org/wikipedia/commons/c/c1/Google_%22G%22_logo.svg" width="50">
                <p style="margin-top: 1rem; color: white;">Search for similar images on Google</p>
            </div>
        </a>
